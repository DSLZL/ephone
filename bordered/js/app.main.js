document.addEventListener('DOMContentLoaded', () => {
        ...
        if (window.EP) { window.EP.state = state; }
        // --- 修正结束 ---
let musicState = { 
    isActive: false, 
    activeChatId: null, 
...
if (window.EP && typeof window.EP.defineStores === 'function') {
    window.EP.defineStores(db);
} else {
    // fallback to inline schema (kept for safety)
    db.version(23).stores({ 
        ...
        callRecords: '++id, chatId, timestamp, customName' // <--�������޸ġ���������� customName
    });
}
        if (window.EP) { window.EP.db = db; }
