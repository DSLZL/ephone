...
async function loadAllDataFromDB() {
    ...
    state.apiConfig = apiConfig || { id: 'main', proxyUrl: '', apiKey: '', model: '', enableStream: false };
...
function parseAiResponse(content) {
    ...
    document.getElementById('hide-stream-switch').checked = state.apiConfig.hideStreamResponse || false;
}
        ...
        switch (msgData.type) {
            ...
            case 'quote_reply':
                ...
                if (originalMessage) {
                    ...
                    aiMessage = { ...baseMessage, content: msgData.reply_content, quote: quoteContext };
                } else {
...
if (enableStream && !isGemini) {
    ...
    if (state.apiConfig.hideStreamResponse) {
        ...
        const contentElement = document.querySelector(`.message-bubble[data-timestamp="${messageTimestamp}"] .content`);
        ...
        for (const msgData of messagesArray) {
            ...
            let aiMessage = null;
            const baseMessage = { role: 'assistant', senderName: msgData.name || chat.name, timestamp: processingTimestamp++ };
            switch (msgData.type) {
                case 'waimai_response':
                    const requestMessageIndex = chat.history.findIndex(m => m.timestamp === msgData.for_timestamp);
                    if (requestMessageIndex > -1) {
    ...
    let geminiConfig = toGeminiRequestData(model,apiKey,systemPrompt, messagesPayload,isGemini);
        ...
        const callHasBeenHandled = await processAndRenderAiMessages(messagesArray, chatId, Date.now());    
            if (callHasBeenHandled && videoCallState.isGroupCall) {
                videoCallState.isAwaitingResponse = false;
                if (videoCallState.participants.length > 0) {
                    startVideoCall();
...
// ▲▲▲ 替换结束 ▲▲▲
            document.getElementById('save-api-settings-btn').addEventListener('click', async () => { state.apiConfig.proxyUrl = document.getElementById('proxy-url').value.trim(); state.apiConfig.apiKey = document.getElementById('api-key').value.trim(); state.apiConfig.model = document.getElementById('model-select').value; state.apiConfig.enableStream = document.getElementById('stream-switch').checked; state.apiConfig.hideStreamResponse = document.getElementById('hide-stream-switch').checked; await db.apiConfig.put(state.apiConfig); 
